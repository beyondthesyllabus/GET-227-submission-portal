  ...
  const handleDownload = async (submission) => {
    ...
    try {
      const res = await fetch(
        `/api/submissions/${submission.referenceCode}/download`,
        {
          headers: { "x-admin-password": token },
        },
      );
      ...
      alert(
        "File download failed. The file may not be available on the server.",
      );
    } finally {
      setDownloading(null);
    }
  ...
  return (
    <div className="admin-dashboard">
      ...
      <div className="admin-top-bar">
        <div>
          ...
          <p className="admin-dashboard-subtitle">
            GET227-Solid Works Modelling and Graphics II Assignment portal
            Submission
          </p>
        </div>
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "center" }}>
          <span style={{ fontSize: "0.85rem", color: "var(--label-color)" }}>
            Signed in as <strong>MechanicalHOD</strong>
          </span>
          <button
      ...
      <div className="admin-stats-grid">
        ...
        <div className="admin-stat-card">
          <div className="admin-stat-value">
            {Object.values(deptCounts).filter((v) => v > 0).length}
          </div>
          <div className="admin-stat-label">Active Departments</div>
        </div>
        <div className="admin-stat-card">
          <div className="admin-stat-value">
            {topDept ? topDept[0].split("/")[0] : "—"}
          </div>
      ...
      <div className="admin-tabs">
        ...
        <button
          className={`admin-tab ${activeTab === "regsearch" ? "active" : ""}`}
          onClick={() => setActiveTab("regsearch")}
        >
          🔎 RegNumber Search
        </button>
      </div>
        ...
        <input
          ...
          placeholder={
            activeTab === "regsearch"
              ...
              : "Search by group name, reference, project title..."
          }
          value={search}
                  ...
                  <tbody>
                    {filteredStudents.map((st, idx) => (
                      <tr key={`${st.referenceCode}-${st.memberNum}`}>
                        ...
                        <td>{st.level}</td>
                        <td>
                          <span className="ref-badge">{st.referenceCode}</span>
                        </td>
                        <td
                ...
                <div style={{ marginTop: "1.5rem" }}>
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={
                        downloading === selectedSubmission.referenceCode
                      }
                    >
                      {downloading === selectedSubmission.referenceCode
                        ? "Downloading..."
                        : "⬇ Download for Assessment"}
                    </button>
                  </div>
                </div>
              ) : (
